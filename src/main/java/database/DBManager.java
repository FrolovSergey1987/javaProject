...
public class DBManager {
    ...
    public static List<Discipline> getAllActiveDisciplines(){
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * FROM students.discipline where status = '1'");
            while (rs.next()){
    ...
    public static void createStudent(Student student){
        try {
            ...
            stmt.execute("INSERT INTO `student` (`name`, `surname`, `group`, `date`) " +
                    "VALUES ('"+student.getName()+"', '"+student.getSurname()+"', '"+student.getGroup()+"', '"+student.getDate()+"');");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    public static void modifyStudent(Student student) {
        try {
            ...
            Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/students?user=root&password=mefA1evica");
            Statement stmt = conn.createStatement();
            stmt.execute("UPDATE `student` SET `surname` = '"+student.getSurname()+"', `name` = '"+student.getName()+"', `group` = '"+student.getGroup()+"', `date` = '"+student.getDate()+"' WHERE (`id` = '"+student.getId()+"');");
    ...
    public static Discipline getDisciplineById(String id) {
        ...
        try {
            ...
            Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/students?user=root&password=mefA1evica");
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM  `students`.`discipline` where status = '1' AND id = " + id);
            while (rs.next()) {
                discipline.setId(rs.getInt("id"));
                discipline.setDiscipline(rs.getString("discipline"));
            }
